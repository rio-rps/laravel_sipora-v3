<div class="modal fade" id="getModalForm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel5" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('cparTrayek.store') }}" class="formData" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-body">
                        <div class="form-group row">
                            <label class="col-sm-4 col-form-label border-bottom">Jenis Trayek Permohonan</label>
                            <div class="col-md-8">
                                <select name="id_par_permohonan" class="form-control select2">
                                    <option value="" selected>--Pilih--</option>
                                    @foreach ($result as $row )
                                    <option value="{{ $row->id_par_permohonan }}">{{ $row->nm_par_permohonan }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="form-body">
                        <div class="form-group row">
                            <label class="col-sm-4 col-form-label border-bottom">Nama Trayek</label>
                            <div class="col-md-8">
                                <textarea name="nm_trayek" class="form-control"></textarea>
                            </div>
                        </div>
                    </div>
