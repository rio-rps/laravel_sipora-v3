                    ...
                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop2" x-placement="bottom-start">
                        ...
                        @if (getLevel() == 1 || getLevel() == 2)
                            @if ($model->status_permohonan == 2)
                                <form method="POST"
                                    action="{{ route('datapermohonan.validasiPermohonan', $model->id_permohonan_izin) }}"
                                    class="dropdown-item formPilih" style="cursor: context-menu;">
                                    @csrf
                                    <input type="hidden" name="_method" value="POST">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}"> <i
                                        class="fa fa-check"></i> Validasi
                                </form>

                                <a class="dropdown-item" title="Ditolak" id="tombolModalForm"
                                    data-url="{{ route('datapermohonan.createTolak', $model->id_permohonan_izin) }}"><i
                                        class="fa fa-close"></i> Tolak</a>
                            @endif
                        @endif
                        @if ($model->status_permohonan == 4)
                            <a class="dropdown-item" title="{{ getLevel() == 3 ? 'Lihat Detail Data' : 'Proses Data' }}"
                                href="{{ route('datapermohonan.kartuInput', Crypt::encrypt($model->id_permohonan_izin)) }}">{!! getLevel() == 3
                                    ? '<i class="fa fa-desktop"></i> Lihat Detail Data'
                                    : '<i class="fa fa-edit"></i> Proses Data' !!}</a>
                        @endif
                        <a class="dropdown-item" title="Dokumen Upload" id="tombolModalForm"
                            data-url="{{ route('datapermohonan.dokumenUpload', $model->id_permohonan_izin) }}"><i
                                class="fa fa-desktop"></i> Dokumen Upload</a>
                        @if ($model->status_permohonan == 5)
                            <a class="dropdown-item" title="Lihit Detail Data"
                                href="{{ route('datapermohonan.kartuInput', Crypt::encrypt($model->id_permohonan_izin)) }}"><i
...
@endif
                        ...
                        <a class="dropdown-item" title="Lihat Permohonan" id="tombolModalForm"
                            data-url="{{ route('datapermohonan.detailView', $model->id_permohonan_izin) }}"><i
                                class="fa fa-eye"></i> Lihat Permohonan</a>
                        @if ($model->status_permohonan == 3)
                            <a class="dropdown-item" title="Informasi ditolak" id="tombolModalForm"
                                data-url="{{ route('datapermohonan.getModalHistoriData', $model->id_permohonan_izin) }}"><i
                                    class="fa fa-comment"></i> Informasi ditolak</a>
                        @endif

                        @if ($model->status_permohonan == 1 || $model->status_permohonan == 2)
                            <form method="POST"
                                action="{{ route('datapermohonan.destroy', $model->id_permohonan_izin) }}"
