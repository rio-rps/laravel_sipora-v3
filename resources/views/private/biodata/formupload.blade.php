<div class="modal fade" id="getModalForm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel5" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            @if ($action ==0)
            <form action="{{ route('upload.storeUpload') }}" class="formDataMultipart" method="POST" enctype="multipart/form-data">
                <input type="hidden" class="form-control" name="id_biodata" value="{{$id}}">
                @else
                <form action="{{ route('upload.updateUpload',$id) }}" class="formDataMultipart" method="POST" enctype="multipart/form-data">
