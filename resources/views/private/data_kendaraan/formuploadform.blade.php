<div class="modal fade" id="getModalForm2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel5" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            @if ($action == 0)
                <form action="{{ route('datakendaraan.storeUpload') }}" class="formDataMultipart" method="POST"
                    enctype="multipart/form-data">
                    <input type="hidden" class="form-control" name="id_kendaraan" value="{{ $id }}">
                @else
                    <form action="{{ route('datakendaraan.updateUpload', $id) }}" class="formDataMultipart"
