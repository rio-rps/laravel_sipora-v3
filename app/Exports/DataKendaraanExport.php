    ...
    WithCustomStartCell
{
    ...
    public function collection()
    {
        ...
        return DataKendaraanModel::with([
            'JBiodata.BadanUsaha',
            'JkendaraanMerek',
            'JkendaraanType'
        ])
    ...
    public function map($row): array
    {
        return [
            $this->no++,
            optional($row->JBiodata)->nm_perusahaan_personal ?? '-',
            optional(optional($row->JBiodata)->BadanUsaha)->nm_badan_usaha ?? 'Personal / Perorangan',
            (optional($row->JkendaraanMerek)->nm_merek_kendaraan ?? '-') . ' / ' .
                (optional($row->JkendaraanType)->nm_type_kendaraan ?? '-'),
            $row->nm_kendaraan ?? '-',
            $row->plat_no_kendaraan ?? '-',

            // Paksa STRING supaya tidak jadi 7,18273E+11
            "'" . ($row->no_rangka ?? '-'),
            "'" . ($row->no_mesin ?? '-'),

            $row->daya_angkut_orang ?? 0,
            $row->daya_angkut_barang ?? 0,
            $row->thn_pembuatan ?? '-',
            $row->status_actived == 1 ? 'Aktif' : 'Tidak Aktif',
        ];
    }
